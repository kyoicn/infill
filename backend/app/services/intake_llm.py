...
2. **打印盘截图（produce）**：拓竹切片软件每个打印盘的预览。

## 关于打印盘截图的细节（极其重要 — 历史上模型常在此犯错）

每张 produce 截图分两部分：**左侧约 2/3** 是 3D 打印盘视图（深灰色背景，上面摆着实体打印件）；**右侧约 1/3** 是耗材面板（白底文字列表）。

### 数件数
- 仔细看**左侧打印盘视图里有几个独立的实体**（一个个数）。**不要**从右侧面板的"耗材次数 / 数量 N"等任何数字反推。
- 例子：盘子上摆着 3 个长方形侧板 → quantity_per_plate = 3。
- 如果某张图你无法明确数清，宁可宽松（取 1）也不要瞎填大数。

### 读总时间字段
- 在**右侧白底面板**找文字"**总时间**"（不是"预计打印时间"，那是另一个字段，常出现在面板下方且字号小）。
- "总时间"标签紧跟的值是 `XhYm` 或 `Xm`（如 `2h43m` / `45m` / `1h2m`），偶尔有秒（`Xh Ym Zs`）。**完整复制这个字符串**到 duration_minutes 后再转换为分钟向上取整。
- 如果两个时间字段都看到，**总时间一定大于或等于预计打印时间**。优先取大的那个并核对标签。
